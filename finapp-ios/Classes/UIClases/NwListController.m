	...
	for(int x = 0; x < varStyles.listComponents.count; x++){
		...
		if([var isEqualToString:@"header"]){
			if([eMobcViewController isIPad]){
				...
				if([[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeLeft || [[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeRight){
					myLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 58, 480, 20)];	
				}else{
					myLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 58, 320, 20)];
				}				
			}
...
-(void) loadThemes {
	if(![varStyles.backgroundFileName isEqualToString:@""]) {
		...
		if([eMobcViewController isIPad]){
			...
			if([[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeLeft || [[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeRight){
				background = [[UIImageView alloc] initWithFrame:CGRectMake(0, 58, 480, 320)];
			}else{
				background = [[UIImageView alloc] initWithFrame:CGRectMake(0, 58, 320, 480)];
			}				
		}
