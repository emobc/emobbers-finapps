...
-(void) loadForm{
	...
	if(data != nil || profile != nil){
        ...
        if([data.dataId isEqualToString:@"login"]){
            if([eMobcViewController isIPad]){
                if([[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeLeft || [[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeRight){
                    background = [[UIImageView alloc] initWithFrame:CGRectMake(0, 00, 1024, 642)];
                }else{
                    background = [[UIImageView alloc] initWithFrame:CGRectMake(0, 00, 768, 898)];
                }				
            }else {
                if([[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeLeft || [[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeRight){
                    background1 = [[UIImageView alloc] initWithFrame:CGRectMake(0, 00, 480, 320)];
                }else{
                    background1 = [[UIImageView alloc] initWithFrame:CGRectMake(0, 00, 320, 480)];
                    
                }				
            }
            
            background1.image = [UIImage imageNamed:@"images/background.png"];
...
-(float) addButtonSubmit:(float) ypos{
    ...
    if ([eMobcViewController isIPad]) {
        ...
        if([data.dataId isEqualToString:@"login"]){
            img = [UIImage imageNamed:[NSString stringWithFormat:@"images/button-gray@2x.png"]];
        }else{
            img = [UIImage imageNamed:[NSString stringWithFormat:@"images/enviar@2x.png"]];
        }
        
    } else { 
        button.frame = CGRectMake(kFormLeftMargin+90, ypos+20, kFormSubmitButtonWidth, kFormSubmitButtonHeight);
		if([data.dataId isEqualToString:@"login"]){
            img = [UIImage imageNamed:[NSString stringWithFormat:@"images/button-gray.png"]];
        }else{
            img = [UIImage imageNamed:[NSString stringWithFormat:@"images/enviar.png"]]; 
        }
