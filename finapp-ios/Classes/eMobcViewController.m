...
- (void)viewDidLoad {
    ...
    NextLevel* StartPointNL = [[NextLevel alloc] initWithData:@"login" dataId:@"login"];
    [self loadNextLevel:StartPointNL];
    [StartPointNL release];
	...
	if([nextLevel.levelId isEqualToString:@"emobc"] && [nextLevel.dataId isEqualToString:@"profile"]){
		...
		if (appLevel != nil) {
			...
			NSString* message;
			 ...
			 message = [NSString stringWithFormat:@"Error de conectividad"];
...
- (void) loadButtonsController:(LoadControlerUtil*)  theControlerUtil{
	...
	self.buttonsController = controller;
	
	[self loadController:buttonsController withAppLevel:theControlerUtil.appLevel];	
	[controller release];
	[theControlerUtil release];
}
