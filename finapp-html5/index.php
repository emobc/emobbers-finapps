...
switch ($activity) {
	...
	*/
		...
		echo'<div class="ui-grid-solo align-center">
			...
			<div class="ui-block-a"><font class="basic_text">'.$texto.'</font>
		</div>';
		if ( isset($image) && $image != ''){
			if(!strrpos("__".$image, "http")){
			$rutaImagen = $assetsPath.$image;
			} else {
			$rutaImagen = $image;
			}
			echo'<div class="ui-block-a"><img src="'.$rutaImagen.'" border="0" style="width: 100%; height: auto;"></div>';
		}
		echo'
		</div>';
		if (isset($nextLevel)) {
			...
			<!--<div class="ui-block-a align-center">
				<a data-role="button" href="index.php?level='.$prevLevel.'&data='.$prevData.'">Anterior</a><br />
			</div>-->
			<div class="ui-block-b align-center">
				<a data-role="button" href="index.php?level='.$nextLevel.'&data='.$nextData.'">Siguiente</a><br />
			</div>
			</div>';
		}
	/**
...
';
			...
			if ( !isset($_POST['action']) || $_POST['action'] == '') {
				...
				header('Location: '.$_SERVER['PHP_SELF'].'?level='.$_POST['level'].'&data='.$nextdata.'');
					...
					curl_close ($ch);
						...
						header('Location: '.$_SERVER['PHP_SELF'].'?level='.$_POST['level'].'&data='.$_POST['data'].'');
			}
		}
	/**
	* @ignore
	*/
		...
		if ($actionUrl != '') {
			...
			<input type="hidden" name="data" value="'.$nextData.'"/>
			';
